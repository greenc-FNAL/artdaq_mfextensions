...
unsigned int
ma_hitmap::capture(qt_mf_msg const& msg, std::string const& src, std::string const& tgt, boost::smatch const& what)
{
	size_t s_idx = 0;
	size_t t_idx = 0;
...
bool ma_hitmap::event(size_t src, size_t tgt, time_t t)
{
	return hitmap[src][tgt].event(t, *cond);
}

int ma_hitmap::find_source(std::string const& src)
{
	idx_t::const_iterator it = src_idx.find(src);
	if (it == src_idx.end())
		return D_NIL;
...
int ma_hitmap::find_target(std::string const& tgt)
{
	idx_t::const_iterator it = tgt_idx.find(tgt);
	if (it == tgt_idx.end())
		return D_NIL;
...
bool ma_hitmap::get_status(ma_cond_domain v) const
{
...
int ma_hitmap::get_alarm_count(ma_cond_domain v, arg_t arg) const
{
...
void ma_hitmap::get_cond_range(ma_cond_domain d, ma_cond_range& src, ma_cond_range& tgt) const
{
	if (domain_is_null(d))
