...
void ma_cond_test_primary::insert_func(std::string const& name, anys_t const& args)
{
	...
	{
		throw std::runtime_error("arguments rejected by test function " + name + "() with an exception:\n" + e.what());
	}

	cond_type = FUNCTION;
}

void ma_cond_test_primary::insert_compare_op(compare_op_t cop, any_t const& v)
{
	op = cop;
