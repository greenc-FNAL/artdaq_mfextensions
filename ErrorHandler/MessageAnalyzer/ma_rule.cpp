...
ma_rule::ma_rule(std::string const& rule_name, std::string const& rule_desc, bool repeat, int holdoff_time)
    : conditions()
    , conditions_idx()
    , primitive_cond()
    , cond_map(NULL)
...
void ma_rule::parse(std::string const& cond_expr, std::string const& alarm_message, ParameterSet const& act_pset, cond_map_t* cond_map_ptr)
{
...
bool ma_rule::recursive_evaluate(ma_domain& value, ma_domain& alarm, ma_domain const& domain, size_t n)
{
	// pre-conditions
...
bool ma_rule::evaluate()
{
	...
	for (ma_domains::const_iterator ait = domains.begin(); ait != domains.end(); ++ait)
	{
...
bool ma_rule::boolean_evaluate(ma_domain& value, ma_domain& alarm, ma_domain const& domain)
{
...
{
	int mask = 0;
	for (ma_actions::const_iterator it = actions.begin(), e = actions.end(); it != e; ++it)
	{
		if ((*it)->exec()) ++mask;
	}
	return mask;
...
void ma_rule::update_notify_list(std::string const& name, arg_t arg)
{
	idx_t::const_iterator it = conditions_idx.find(name);
