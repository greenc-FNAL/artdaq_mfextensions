...
static void
insert_cond_arg(ma_domain_cond& cond, std::string const& name, char arg, ma_rule* rule)
{
	...
	cond.insert_cond_arg(rule->get_cond_idx(name), (arg == 's') ? SOURCE : TARGET, rule->get_cond_size());
}
...
template<class FwdIter, class Skip>
novadaq::errorhandler::domain_expr_parser<FwdIter, Skip>::domain_expr_parser(ma_rule* rule)
    : domain_expr_parser::base_type(domain_expr)
{
	...
	domain_cond =
	    (lit('(') >> domain_expr[phx::bind(&insert_domain_expr, ql::_val, ql::_1)] >> lit(')'))  // '(' >> expr >> ')'
	    |
	    ((
	         (
	             key[ql::_a = ql::_1] >> ".$" >> char_("st")
	                                                 [phx::bind(&insert_cond_arg, ql::_val, ql::_a, ql::_1, rule)]) %
	         '=') >>
	     -('=' >> (str[phx::bind(&insert_str_cond, ql::_val, ql::_1)] | "ANY")))  // cond1.$x = cond2.$y = ... -( = "str_cond" )
	                                                                              // cond1.$x = cond2.$y = ... -( = ANY )
	    ;
...
static void
insert_primitive_cond(ma_boolean_cond& cond, std::string const& name, ma_rule* rule)
{
...
static void
insert_ext_func(ma_boolean_cond& cond, std::string const& function, std::string const& name, char cond_arg, std::vector<boost::any> const& func_args, ma_rule* rule)
{
	cond.insert_ext_func(rule->insert_condition_ptr(name, false /*non-primitive*/), (cond_arg == 's') ? SOURCE : ((cond_arg == 't') ? TARGET : NONE), func_args, function);
}
...
static void
insert_compare_op_string(ma_boolean_cond& cond, compare_op_t op, std::string rhv)
{
...
template<class FwdIter, class Skip>
novadaq::errorhandler::boolean_expr_parser<FwdIter, Skip>::boolean_expr_parser(ma_rule* rule)
    : boolean_expr_parser::base_type(boolean_expr)
{
	...
	boolean_cond =
	    (lit('(') >> boolean_expr[phx::bind(&insert_boolean_expr, ql::_val, ql::_1)] >> ')')  // '(' >> expr >> ')'
	    |
	    (key[ql::_a = ql::_1] >> lit('(') >> key[ql::_d = ql::_1] >> -(".$" >> char_("st")[ql::_b = ql::_1]) >> -(',' >> args[ql::_e = ql::_1]) >> lit(')')[phx::bind(&insert_ext_func, ql::_val, ql::_a, ql::_d, ql::_b, ql::_e, rule)]

	     >> -(compare_op[ql::_c = ql::_1] >> (double_[phx::bind(&insert_compare_op_double, ql::_val, ql::_c, ql::_1)] | bool_[phx::bind(&insert_compare_op_bool, ql::_val, ql::_c, ql::_1)] | str[phx::bind(&insert_compare_op_string, ql::_val, ql::_c, ql::_1)])))  // custom_function(cond.$st)
	    |
	...
	arg = double_[ql::_val = ql::_1] | bool_[ql::_val = ql::_1] | str[ql::_val = ql::_1];
	...
	compare_op = lit("==")[ql::_val = CO_E] | lit("!=")[ql::_val = CO_NE] | lit("<=")[ql::_val = CO_LE] | lit(">=")[ql::_val = CO_GE] | lit("<")[ql::_val = CO_L] | lit(">")[ql::_val = CO_G];
}
...
bool novadaq::errorhandler::parse_condition_expr(std::string const& s, ma_rule* rule)
{
	...
	bool b = qi::phrase_parse(
	             begin, end, boolean_p[phx::bind(&set_boolean_expr, ql::_1, rule)] >> -("WHERE" >> domain_p[phx::bind(&set_domain_expr, ql::_1, rule)]), space) &&
	         begin == end;
...
static void
insert_test_primary(ma_cond_test_andexpr& andexpr, ma_cond_test_primary const& primary)
{
	andexpr.insert(primary);
}

static void
insert_test_andexpr(ma_cond_test_expr& expr, ma_cond_test_andexpr const& andexpr)
{
	expr.insert(andexpr);
}
...
static void
insert_test_func(ma_cond_test_primary& primary, std::string const& function, std::vector<boost::any> const& func_args)
{
	primary.insert_func(function, func_args);
}

static void
insert_test_compare_op(ma_cond_test_primary& primary, compare_op_t op, boost::any const& rhv)
{
...
template<class FwdIter, class Skip>
novadaq::errorhandler::cond_test_expr_parser<FwdIter, Skip>::cond_test_expr_parser()
    : cond_test_expr_parser::base_type(test_expr)
{
	...
	test_primary =
	    (lit('(') >> test_expr[phx::bind(&insert_test_expr, ql::_val, ql::_1)] >> lit(')')) |
	    (key[ql::_a = ql::_1] >> lit('(') >> -values[ql::_b = ql::_1] >> lit(')')[phx::bind(&insert_test_func, ql::_val, ql::_a, ql::_b)] >> -(compare_op[ql::_c = ql::_1] >> value[phx::bind(&insert_test_compare_op, ql::_val, ql::_c, ql::_1)]));

	values = (value % ',');

	value = (int_[ql::_val = ql::_1] >> !char_(".eE")) | double_[ql::_val = ql::_1] | bool_[ql::_val = ql::_1] | str[ql::_val = ql::_1];
...
bool novadaq::errorhandler::parse_condition_test(std::string const& s, ma_cond_test_expr& expr)
{
	typedef std::string::const_iterator iter_t;
	typedef ascii::space_type ws_t;
