...
template<typename T>
bool compare(compare_op_t op, T v, T rhv)
{
	switch (op)
	{
		case CO_L:
...
void ma_boolean_cond::insert_ext_func(cond_idx_t ci, arg_t arg, anys_t const& func_args, std::string const& function)
{
	cond_arg.first = ci;
	cond_arg.second = arg;
	cond_type = FUNCTION;
...
bool ma_boolean_cond::evaluate(ma_domain& value, ma_domain& alarm, ma_domain const& domain) const
{
	if (cond_type == EXPR)
	{
		// expression must not be null
	...
	if (cond_type >= FUNCTION)
	{
		...
		else
		{
			alarm[cond_idx.second] =
			    ma_cond_domain(cond_idx.first->find_source(cond_idx.first->get_msg_source()), cond_idx.first->find_target(cond_idx.first->get_qt_mf_msgarget()));
		}

		// evaluate
		boost::any v = ext_func->evaluate(*(cond_idx.first), domain[cond_idx.second]);
		double d;
		bool b;
		std::string s;
