...
bool parse_msg_ref(std::string const& s, ma_rule const* rule, std::vector<cond_arg_t>& symbols)
{
	size_t pos = s.find('.');
...
bool parse_msg(std::string const& s, ma_rule const* rule, std::string& stripped_msg, std::vector<size_t>& insert_pos, std::vector<cond_arg_t>& symbols)
{
	size_t old = 0;
	size_t pos = s.find("${");
	size_t ins = 0;
