...
public:
	...
	void
	parse(std::string const& cond_expr, std::string const& alarm_message, fhicl::ParameterSet const& actions, cond_map_t* cond_map_ptr);
...
private:
	...
	bool
	recursive_evaluate(ma_domain& value, ma_domain& alarm, ma_domain const& domain, size_t n);

	// evaluate the boolean expression with a given set of inputs
	//   value:  the input values for each condition
	bool
	boolean_evaluate(ma_domain& value, ma_domain& alarm, ma_domain const& domain);
